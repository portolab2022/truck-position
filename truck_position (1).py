...
# Define correct position criteria
CORRECT_X_RANGE = (0.4, 0.7)  # Range for front centroid x
CORRECT_Y_RANGE = (0.4, 0.7)  # Range for front centroid y
ALIGNMENT_THRESHOLD = 0.2  # Maximum allowed difference in x-coordinates for alignment
...
for filename in os.listdir(image_folder):
    if filename.endswith('.jpg') or filename.endswith('.png'):
        ...
        for result in results[0].boxes:
            ...
            x, y, w, h = result.xywh[0]
            centroid_x, centroid_y = x / image.shape[1], y / image.shape[0]  # Normalize
            x1 = max(0, int(x - w / 2))  # Top-left x, clipped to 0
            y1 = max(0, int(y - h / 2))  # Top-left y, clipped to 0
            x2 = min(image.shape[1], int(x + w / 2))  # Bottom-right x, clipped to width
            y2 = min(image.shape[0], int(y + h / 2))  # Bottom-right y, clipped to height

            if class_id == 0:  # Front
                info.append(f"Front detected at (centroid: {centroid_x:.2f}, {centroid_y:.2f})")
                cv2.rectangle(image_bgr, (x1, y1), (x2, y2), (0, 255, 0), 2)  # Green box
                cv2.circle(image_bgr, (int(x), int(y)), 5, (0, 0, 255), -1)  # Red centroid
                front_centroid = (centroid_x, centroid_y)
            elif class_id == 1:  # Side
                info.append(f"Side detected at (centroid: {centroid_x:.2f}, {centroid_y:.2f})")
                cv2.rectangle(image_bgr, (x1, y1), (x2, y2), (255, 0, 0), 2)  # Blue box
                cv2.circle(image_bgr, (int(x), int(y)), 5, (0, 0, 255), -1)  # Red centroid
                side_coords = (x1, y1, x2, y2)
                side_detected = True

        # Check position with fallback for front-only detection
        if front_centroid:
            centroid_x, centroid_y = front_centroid
            is_within_front_bounds = (CORRECT_X_RANGE[0] <= centroid_x <= CORRECT_X_RANGE[1] and
                                    CORRECT_Y_RANGE[0] <= centroid_y <= CORRECT_Y_RANGE[1])
            is_within_image_bounds = (0 <= x1 <= image.shape[1] and 0 <= y1 <= image.shape[0] and
                                    0 <= x2 <= image.shape[1] and 0 <= y2 <= image.shape[0])

            if side_coords:
                is_within_side_bounds = (0 <= side_coords[0] <= image.shape[1] and 0 <= side_coords[1] <= image.shape[0] and
                                       0 <= side_coords[2] <= image.shape[1] and 0 <= side_coords[3] <= image.shape[0])
                side_centroid_x = (side_coords[0] + side_coords[2]) / 2 / image.shape[1]
                alignment_check = abs(centroid_x - side_centroid_x) <= ALIGNMENT_THRESHOLD
                if is_within_front_bounds and is_within_side_bounds and alignment_check:
                    position_status = "Correct"
                else:
                    position_status = "Incorrect"
            else:
                # Fallback: Check front only if side is not detected
                if not side_detected:
                    info.append("Side not detected in this image")
                if is_within_front_bounds and is_within_image_bounds:
                    position_status = "Correct"
                else:
                    position_status = "Incorrect"
...
# Define criteria
CORRECT_X_RANGE = (0.4, 0.7)
CORRECT_Y_RANGE = (0.4, 0.7)
...
for filename in os.listdir(image_folder):
    if filename.endswith('.jpg') or filename.endswith('.png'):
        ...
        for result in results[0].boxes:
            class_id = int(result.cls[0])
            x, y, w, h = result.xywh[0]
            centroid_x, centroid_y = x / image.shape[1], y / image.shape[0]
            x1, y1 = max(0, int(x - w / 2)), max(0, int(y - h / 2))
            x2, y2 = min(image.shape[1], int(x + w / 2)), min(image.shape[0], int(y + h / 2))
            confidence = result.conf[0].item()

            if class_id == 0:  # Front
                info.append(f"Front detected at (centroid: {centroid_x:.2f}, {centroid_y:.2f}, conf: {confidence:.2f})")
                cv2.rectangle(image_bgr, (x1, y1), (x2, y2), (0, 255, 0), 2)
                cv2.circle(image_bgr, (int(x), int(y)), 5, (0, 0, 255), -1)
                front_centroid = (centroid_x, centroid_y)
            elif class_id == 1:  # Side
                info.append(f"Side detected at (centroid: {centroid_x:.2f}, {centroid_y:.2f}, conf: {confidence:.2f})")
        ...
        plt.figure(figsize=(10, 5))
        plt.imshow(image_display)
        plt.title(f"Image: {filename}\n{' | '.join(info) if info else 'No detections'}\nPosition: {position_status}")
        plt.axis('off')
        plt.show()
